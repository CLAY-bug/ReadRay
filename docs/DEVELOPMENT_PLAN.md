...
当前已完成 SQLite schema、迁移、解释成功后自动保存、读取/搜索/筛选/删除接口及测试，并将已验收的“记忆”页面接到真实分页、关键词搜索、类型筛选和单条详情读取。正式页面不聚合或伪造重复出现次数；删除 command 已保留，但本轮未增加未经设计验收的删除入口。
...
阶段一和阶段二已完成，阶段三实现已完成并等待真实桌面全交互人工验收。
...
1. 定义学习记录 schema 和数据迁移策略。
2. 在 ExplanationCard 校验通过后保存查询事件。
3. 实现分页读取、关键词搜索、类型筛选和单条删除。
4. 为迁移、序列化、查询和错误路径增加测试。
5. 数据协议稳定后，单独进行学习记录窗口的 UI 设计与实现。

上述 1–5 已实现；进入阶段四前，需在真实 `pnpm tauri dev` 窗口中验收 overlay 新记录即时刷新、列表、搜索、四类筛选、详情、空/错误状态、分页和侧栏展开/折叠。

...
状态：任务 0/1/2/3/4/5/6 全部完成并通过评审（任务 3/4/5/6 已正式收口，任务 3/4 含真实 Tauri 使用验收）；阶段八点五 Agent Runtime 升级完成并已合入 main
...
任务 0/1/2/3/4/5/6 已全部通过评审并正式收口（任务 3/4 含真实 Tauri 人工验收；任务 5 为预算驱动长上下文投影 + 方案 A 最简折叠兜底；任务 6 Writing Coach 已接入共享 Runtime 并保留写作权威，见实施记录），**阶段八点五 Agent Runtime 升级全部完成并已合入 main**。任务 4 的"编辑并重新生成"已按 §18.4 覆盖式语义正式开放（不物理覆盖旧问题/旧回答）；Web Search 已通过 Wikipedia provider 开放给真实会话（覆盖范围以工具描述为准）；Writing Coach 检查/问答已复用同一 Runtime 内核（写作专属空工具集，默认不联网）。已知问题：写作检查曾返回 0 个问题（见任务 6 实施记录），待后续真实复验与处理，不阻塞整体 Preview 发布。

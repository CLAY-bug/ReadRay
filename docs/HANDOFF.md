...
- 阶段八点五（Agent Runtime）：任务 0 协议、任务 1 Rust Agent Kernel、任务 2 通用对话接入与 SQLite run/step 恢复、任务 3 自动联网纵切均已通过或完成实现（任务 1/2 各含一轮修复复审；任务 3 已完成实现、验证与浏览器预览验收，正待调度者评审）。实现集中在 `agent-runtime` 分支（原 `feat/agent-runtime-task1` 改名），按约定任务 1-6 全部完成评审后再一次性合入 main，main 保持干净（当前仅含 learning_records 测试修复 `a15db94`）。下一步进入任务 4 日常使用交互。
...
- 待验收项：旧非 Agent 路径（`send_quick_ai_message`/`send_quick_ai_message_streaming`）保留为受控回退；DeepSeek Responses API 因 live spike 返回 400 未确认，任务 2 及以后继续以 chat/completions 为依据；任务 2 全部验收由离线测试达成（fake gateway + 真实 SQLite），尚未做真实 Tauri/DeepSeek 人工验收（首次接入后的真实请求验收应纳入任务 3 或单独停点）。
- 分支纪律：任务 1-6 全部在 `agent-runtime` 分支累计实现（任务 1 起名 `feat/agent-runtime-task1`，2026-08-17 改名），每个任务经调度者评审后进入下一个；全部完成后再一次性合入 main。当前分支 tip `54d6b94`（任务 2 评审修复），main 仅含 `a15db94`（learning_records v17 回填测试修复已 cherry-pick 并推送，main 全绿 303/0）。任务 2 收口时验证：全量 Rust lib 334/0、agent_runtime 84/0、fmt/check/diff 全绿。

### 阶段八点五任务 3（自动联网纵切，已完成实现待评审）
...
- 当前实现仍维持“不声称能访问本地学习记录/联网/长期记忆”的诚实边界；`docs/AGENT_UPGRADE.md` 中无网络/无工具是已完成旧任务的历史范围。未来联网与工具调用只按 `docs/AGENT_RUNTIME_UPGRADE_PLAN.md` 分任务接入；“重新生成”继续沿用覆盖式语义，但尚未实现。
...
- **Agent 后续边界**：Quick AI 已支持流式输出、停止/重试、白名单 Markdown 渲染和组合式系统提示词；阶段八点五任务 0/1/2 已完成并通过评审，任务 3 自动联网纵切（Wikipedia 受控搜索 + 受控网页抓取 + 来源卡片）已完成实现待评审（集中在 `agent-runtime` 分支，任务 1-6 全部完成后再一次性合入 main）。任务 3 完成后对话真实会话已具备模型自主搜索能力；继续从任务 4 日常使用交互逐项实现、验证、审查并停点验收。记忆注入属于阶段九；回答重新生成、超 8K 续写和长上下文按新计划的后续任务处理，不因文档建立自动并入正式能力。

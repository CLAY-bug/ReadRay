...
- 当前状态：阶段一至阶段六已经完成；阶段七设置页的 Key 管理和第一批“官方余额、打开数据目录、SQLite 备份”已经通过审核与真实 Tauri 验收。第二批“ReadRay 本地 Token 使用量、备份本地日期修复”已实现，正等待独立审核与真实 Tauri 人工验收。第三批设置和桌面生命周期仍未实现，因此阶段七未完成。
- 当前路线：Windows 原生，Tauri + React + TypeScript + Rust + SQLite。
- 下一步：独立审核并人工验收本地 Token 使用量和中国时区零点附近的备份默认日期；字体、发送快捷键与桌面生命周期继续留给后续独立任务，不与本批混做。
...
### 阶段七：设置页面（第二批待独立审核与人工验收）

- 已按 `design-open-design/readray-settings.html` 把设置入口接入现有 `MainAppShell` / `MainSidebar`，没有复制外壳。视觉返修后保留通用、外观、AI 服务、数据、关于五类导航，以及设计稿的 192px 分类栏、820px 内容列、卡片层级和页面密度。Key 配置/清除、官方余额、四档本地 Token 使用量、SQLite 概览、打开数据目录、SQLite 备份和随包字体许可证弹窗现已形成真实闭环；当前主题、字体、语言和固定快捷键只读展示，桌面生命周期操作继续诚实禁用。
...
- 本轮自动验证通过：设置前端 11 项、会话前端回归 18 项、写作前端回归 25 项；本次统计起点聚焦 Rust 3 项及完整 Rust 88 项通过，2 项真实 DeepSeek 联网测试按既有 ignored 标记跳过；`pnpm build`、`cargo fmt --check`、`cargo check`、RESOURCE_MAP YAML 解析和 `git diff --check` 均通过。没有使用浏览器、Computer Use 或 Tauri 窗口替代人工验收。
- 尚未完成：第二批真实 DeepSeek usage 写入、四档范围跨本机零点、失败重试、重启后统计恢复、升级既有数据库和备份默认日期的真实 Tauri 人工验收；第三批字体/字号与发送快捷键、桌面生命周期仍未实现。阶段七不得因第二批代码完成而标记完成。

## 下一步

阶段六已经完成，阶段七第一批已经验收；第二批本地 Token 使用量与备份本地日期修复正等待独立审核和真实 Tauri 人工验收。阶段七状态不变，第三批设置和桌面生命周期留给后续独立任务。

- 本批独立审核应重点检查 usage 解析发生在业务结构校验前、SQLite 写入失败只影响统计、候选 Key/余额明确排除、v5 migration 不回填历史、任意范围统计开始时间来自首条真实记录，以及四档本机日历边界和总览/分类聚合一致性；余额还需检查自动刷新清理、失败保留旧值和迟到请求隔离。
- 用户人工验收在真实 Tauri 中覆盖解释查询、Quick AI、写作各产生一次真实 usage；无效业务 JSON 仍计数；数据库暂时不可写不影响模型结果；今天/7 天/30 天/全部切换、失败重试、重启恢复和中国时区零点附近的备份默认日期。人工验收前不要把第二批写成已验收。
...
- **阶段七范围**：Key 管理及第一批余额、目录、备份已经验收；第二批本地 Token 使用量与备份本地日期修复已实现但尚待独立审核与真实窗口验收。字体/字号、发送快捷键未接线。主窗口关闭后仍只隐藏并保留后台快捷键，托盘、单实例、开机启动、关闭策略和重新显示主窗口入口尚未形成完整桌面生命周期，留给后续独立任务。

...
- 当前状态：阶段一至阶段六已经完成；阶段七中的“设置页面”已完成实现和自动验证，正等待独立审核与用户执行的真实 Tauri 人工验收。桌面生命周期仍未实现，因此阶段七未完成。
- 当前路线：Windows 原生，Tauri + React + TypeScript + Rust + SQLite。
- 下一步：先独立审核并人工验收设置页面；通过后另开独立任务处理托盘、单实例、开机启动、关闭策略和重新打开主窗口，不与本轮设置页返修混做。
...
- 已按 `design-open-design/readray-settings.html` 把设置入口接入现有 `MainAppShell` / `MainSidebar`，没有复制外壳。视觉返修后恢复通用、外观、AI 服务、数据、关于五类导航，以及设计稿的 192px 分类栏、820px 内容列、卡片层级和页面密度。形成真实闭环的 Key 配置/清除、SQLite 概览和随包字体许可证弹窗可操作；当前主题、字体、语言和固定快捷键只读展示，余额/使用量、目录打开、备份及桌面生命周期操作明确禁用且不报告成功。
...
- 设置快照还从 Tauri 运行时返回实际模型、应用版本和数据目录，并直接从 SQLite 读取学习记录、Quick AI 会话、写作文档计数；没有从前端列表推算。数据目录当前只读显示；目录打开、备份/恢复/清空/全量导出保留设计信息位置但不可操作。
- 设置页响应式规则继续以应用容器为准：1440×900 使用 192px 分类栏；840×600 时分类改为顶部横向，900px 以下表单和数据行纵向排列；全局侧栏仍只按用户操作在 252/72px 间切换。
- 本轮自动验证通过：设置前端 4 项、会话前端回归 18 项、写作前端回归 25 项；API Key 状态一致性聚焦 Rust 8 项及完整 Rust 76 项通过，2 项真实 DeepSeek 联网测试按既有 ignored 标记跳过；`pnpm build`、`cargo fmt --check`、`cargo check` 均通过。没有使用浏览器、Computer Use 或 Tauri 窗口替代人工验收。
...
阶段六已经完成，阶段七的设置页实现已等待独立审核和真实 Tauri 人工验收；桌面生命周期留给后续独立任务。

- 独立审核应重点检查 Windows Credential Manager FFI 生命周期、Key 不进入前端返回/日志/SQLite、验证成功后才替换、清除对 `.env` 回退的抑制，以及页面异步身份守卫。
- 用户人工验收按设置页清单覆盖首次配置、错误 Key、更新失败保留旧 Key、更新成功、清除、重启恢复、三条 AI 链路和窄窗口布局；验收完成前不要把设置子任务写成已验收。
...
- **阶段七范围**：设置页面已实现但尚待独立审核与真实窗口验收；主窗口关闭后仍只隐藏并保留后台快捷键。托盘、单实例、开机启动、关闭策略和重新显示主窗口入口尚未形成完整桌面生命周期，留给后续独立任务。

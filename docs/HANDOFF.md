...
- 阶段八点五（Agent Runtime）：任务 0 协议、任务 1 Rust Agent Kernel、任务 2 通用对话接入与 SQLite run/step 恢复、任务 3 自动联网纵切、任务 4 日常使用交互、任务 5 长上下文与 compaction 均已通过调度者独立评审并正式收口（任务 3/4 含真实 Tauri 使用验收：多轮追问、失败后发新消息、联网来源卡片、"能联网吗"据实回答、编辑并重新生成、截断提示；任务 5 为预算驱动长上下文投影 + 方案 A/方案三最简折叠兜底，方案范围按用户确认简化，见章节）。实现集中在 `agent-runtime` 分支（原 `feat/agent-runtime-task1` 改名），按约定任务 1-6 全部完成评审后再一次性合入 main，main 保持干净。下一步进入任务 6 Writing Coach 适配。

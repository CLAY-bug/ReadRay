...
当前只提供内置 `ReadRay Default` 浅色主题。协议支持浅色和深色，但设置页只展示主题实际声明并通过校验的模式。Codex、Obsidian 或其他来源以后必须通过各自独立 adapter 转换为 ReadRayThemeV1，本协议不承诺兼容任何外部主题格式。
...
- `id`：1–64 个字符，只允许小写 ASCII 字母、数字和连字符，且必须以字母或数字开头；`readray-default` 保留给内置主题。
- `name`：1–80 个字符。
- `version`：1–32 个字符。
- `author`：1–80 个字符。
...
- SQLite v8 只保存规范化 manifest、light/dark 颜色和解析警告，不保存原始 CSS。内置默认主题只存在于代码资源中。

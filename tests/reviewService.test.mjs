...
test("页面采用响应式活跃书架和单卡专注阅读，不保留分页、瀑布流或装饰竖线", async () => {
  const pageSource = await readFile("src/components/ReviewPage.tsx", "utf8");
  const styles = await readFile("src/styles/review-page.css", "utf8");
  assert.match(pageSource, /REVIEW_SHELF_MAX_COLUMNS\s*=\s*3/);
  assert.match(pageSource, /REVIEW_SHELF_CAPACITY\s*=\s*REVIEW_SHELF_MAX_COLUMNS \* 2/);
  assert.match(pageSource, /function reviewShelfCapacity\(columnCount/);
  assert.match(pageSource, /getPropertyValue\("--rr-review-column-count"\)/);
  assert.match(pageSource, /new ResizeObserver/);
  assert.match(pageSource, /observer\.observe\(feedElement\)/);
  assert.match(pageSource, /Array<number \| undefined>/);
  assert.match(pageSource, /!card\.attempt \|\| card\.feedItemId === activeFeedItemId/);
  assert.match(pageSource, /shelfCardsById\.has\(feedItemId\)/);
  ...
  assert.doesNotMatch(styles, /\.rr-review-pagination|\.rr-review-feed-note/);
  ...
  assert.match(pageSource, /LIST_MARKER_LINE/);
  ...
  assert.doesNotMatch(pageSource, /长期记忆与个性化排序留到阶段九/);

...
最后更新：2026-08-12
状态：已完成（2026-08-12）；任务 1～4 已通过实现、必要验证和用户真实使用验收；原任务 5 不再单独执行，完整回归并入未来正式发布前检查
...
single-flight 在第一次 `await` 前按 cache key 同步加入，SQLite lookup、provider、一次 usage 与 cache upsert 位于共享 future；每个调用者继续持有独立 requestKey/generation 和 authority 锁内 cache-commit 身份。leader 取消不影响有效 follower，最后 waiter 取消会中止 provider 且不写缓存；共享结果返回后每个仍有效请求分别重绑、复验并保存自己的真实 learning record，cache hit 不伪造 usage。
...
状态：不再单独执行（2026-08-12）；其完整回归清单保留为未来正式发布前检查参考

### 收口决定

- 任务 5 不包含新的产品功能，原计划仅集中重复运行跨模块自动回归并再次确认真实体验。
- 任务 1～4 已分别完成代码审查、直接相关的聚焦验证和用户真实使用验收，用户确认当前速度、质量、中文反查与缓存体验符合预期。
- 基于实际投入产出，本轮不再为形式上的阶段完整重复运行全量测试，也不把未运行的任务 5 记作“测试通过”。
- 下列自动回归清单保留，用于未来正式发布、参赛演示打包或相关模块发生较大改动时统一执行；它不再是本轮划词优化完成的前置条件。
